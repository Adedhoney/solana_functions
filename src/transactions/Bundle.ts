...
export const sendBundle = async (
    ...
    while (!isLeaderSlot) {
        const next_leader =
            await sc.getNextScheduledLeader()
        const num_slots =
            next_leader.nextLeaderSlot -
            next_leader.currentSlot
        isLeaderSlot = num_slots <= 2
        console.log(
            `next jito leader slot in ${num_slots} slots`
        )
    ...
    let maybeBundle = bundle.addTransactions(
        ...transactions
    )
